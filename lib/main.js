...
const ajtry = {
  ...
  printTest() {
    ...
    this.check.forEach((e) => console.log(e));
    this.observations.forEach((e) => console.log(e));

    const ttf = this.testFail
      ? `\x1b[31m\x1b[1m${this.testFail} failed\x1b[0m,`
      : "";
    const ttp = this.testPass
      ? `\x1b[32m\x1b[1m${this.testPass} passed\x1b[0m,`
      : "";
    const tsf = this.suiteFail
      ? `\x1b[31m\x1b[1m${this.suiteFail} failed\x1b[0m,`
      : "";
    const tsp = this.suitePass
      ? `\x1b[32m\x1b[1m${this.suitePass} passed\x1b[0m,`
      : "";

    console.log(
      `\nTest Suites: ${tsf} ${tsp} ${this.suiteFail + this.suitePass} total.`
    );
    console.log(
      `Test:        ${ttf} ${ttp} ${this.testFail + this.testPass} total.`
    );

    console.log(
      `Time:        \x1b[33m\x1b[1m${(performance.now() - ti).toFixed(
        2
      )} ms\x1b[0m`
    );
    console.log("Ran all tests suites.");
  },
};
